...
import { BadgeDollarSign, CheckCircle2, Clock3, FileText, TrendingUp } from 'lucide-react';
...
export function DashboardPage() {
  ...
  const cards = [
    [FileText, 'Mis cotizaciones', data ? metrics.misCotizaciones : '...', 'bg-[#f8fafc] text-[#414141] border border-[#d9d9d9]', '/cotizaciones'],
    [CheckCircle2, 'Cotizaciones pendientes', data ? metrics.cotizacionesPendientes : '...', 'bg-[#f8fafc] text-[#414141] border border-[#d9d9d9]', '/cotizaciones?estado=enviada'],
    [BadgeDollarSign, 'Mis ventas del mes', data ? money(metrics.misVentasMes) : '...', 'bg-[#f8fafc] text-[#414141] border border-[#d9d9d9]', '/cotizaciones?estado=aprobada'],
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          ...
          <article className="lg:col-span-2 rounded-2xl border border-[#d9d9d9] bg-white p-6 shadow-xs">
            <div className="flex items-center justify-between border-b border-[#d9d9d9] pb-4 mb-6">
              <div>
                <h3 className="text-sm font-semibold text-[#414141]">Mis ventas del mes</h3>
                <p className="text-[11px] text-neutral-400">Historial mensual de ventas aprobadas</p>
              </div>
              <TrendingUp className="h-4 w-4 text-[#414141]" />
            </div>
            <div className="flex h-56 items-end justify-around gap-2 px-2 pt-2">
              {(data?.misVentasPorMes || [{ mes: 'Actual', total: 0 }]).map(row => (
          ...
          <article className="rounded-2xl border border-[#d9d9d9] bg-white p-6 shadow-xs flex flex-col justify-between">
            <div className="flex items-center justify-between border-b border-[#d9d9d9] pb-4 mb-6">
              <div>
                <h3 className="text-sm font-semibold text-[#414141]">Mis cotizaciones</h3>
                <p className="text-[11px] text-neutral-400">Estados de mis cotizaciones</p>
              </div>
              <CheckCircle2 className="h-4 w-4 text-[#414141]" />
            </div>
        ...
        <section>
          <article className="rounded-2xl border border-[#d9d9d9] bg-white p-6 shadow-xs flex flex-col justify-between">
            <div className="flex items-center justify-between border-b border-[#d9d9d9] pb-4 mb-4">
              <div>
                <h3 className="text-sm font-semibold text-[#414141]">Mis últimas cotizaciones</h3>
