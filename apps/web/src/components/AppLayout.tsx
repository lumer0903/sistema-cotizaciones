...
const NAV_ITEMS: {
  ...
  { href: '/productos', icon: Package, label: 'Inventario', permission: 'productos' },
  { href: '/precios', icon: Tag, label: 'Precios e Historial', permission: 'consulta_precios' },
  {
    href: '/cotizaciones',
    icon: FileText,
    label: 'Cotizaciones',
    ...
    children: [
      { href: '/cotizaciones/crear', label: 'Crear cotización' },
      { href: '/cotizaciones', label: 'Mis cotizaciones' },
    ],
  },
  {
    href: '/ventas',
    ...
    permission: 'ventas',
    children: [
      { href: '/ventas/crear', label: 'Crear venta' },
      { href: '/ventas', label: 'Mis ventas' },
    ],
  },
  { href: '/cobranza', icon: Search, label: 'Cobranza', permission: 'cobranza' },
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-gray-200 bg-white/80 backdrop-blur-md px-6 md:px-8">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-xs font-semibold text-gray-900">{usuario?.nombre || 'Usuario'}</span>
              <span className="text-[10px] font-semibold tracking-wider text-gray-400 uppercase">
                {usuario?.rol ? ROLE_NAMES[usuario.rol] : 'Rol'}
              </span>
            </div>
